...
@Component({
      ...
      <div class="fc-card" [class.fc-card--flipped]="showAnswer()">
        ...
        <div class="fc-face fc-face--front card-section">
          ...
          <div class="fc-question-text">
            <h3 style="font-size: 17px; font-weight: 600; color: var(--color-text-primary); line-height: 1.6; margin: 0;">
              {{ question().text }}
            </h3>
          ...
          @if (question().context) {
            <div class="fc-context">
              <span style="font-size: 13px; font-weight: 600; color: var(--color-text-secondary);">Contexto:</span>
              <span style="font-size: 13px; color: var(--color-text-muted); line-height: 1.5;">{{ question().context }}</span>
            </div>
          }
          ...
          <div class="fc-front-options" (click)="$event.stopPropagation()">
            @for (option of question().options; track option.id) {
              <button class="fc-front-option"
                [class.fc-front-option--selected]="selectedOption() === option.id"
                [class.fc-front-option--correct]="answered() && option.id === question().correctOptionId"
                [class.fc-front-option--wrong]="answered() && selectedOption() === option.id && option.id !== question().correctOptionId"
                [disabled]="answered()"
                (click)="selectOption(option.id)">
                <span class="fc-option-label">{{ getOptionLabel(option.order) }}</span>
                <div style="flex: 1; text-align: left;">
                  <div style="font-size: 13px; line-height: 1.4; color: #04202C;">{{ option.text }}</div>
        ...
        <div class="fc-face fc-face--back card-section">
          <div class="fc-back-header">
            <span class="font-display text-forest" style="font-size: 16px; font-weight: 700;">Respuesta</span>
            <span class="tag font-mono">{{ question().domainCode }}</span>
          </div>

          <div class="stack" style="margin-bottom: 16px;">
            @for (option of question().options; track option.id) {
              <div class="fc-option" [class.fc-option--correct]="option.id === question().correctOptionId">
                <span class="fc-option-label" [class.text-forest]="option.id === question().correctOptionId">
                  {{ getOptionLabel(option.order) }}
                </span>
                <div style="flex: 1;">
                  <div style="font-size: 14px; line-height: 1.5;">{{ option.text }}</div>
                  @if (option.textEs) {
                    <div style="font-size: 12px; color: #7D8F84; line-height: 1.3; margin-top: 2px;">{{ option.textEs }}</div>
                  }
                </div>
          ...
          <div class="alert alert-info" style="margin-bottom: 12px;">
            ...
            @if (question().explanationEs) {
              ...
              <div style="font-size: 12px; line-height: 1.5; color: var(--color-text-muted); border-top: 1px solid var(--color-border-subtle, #EFF2F0); padding-top: 8px;">
  ...
  styles: [`
    ...
    .fc-question-es:empty { display: none; }
